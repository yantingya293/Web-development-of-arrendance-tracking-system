...
## 七、外部渗透测试记录（2026-09-17 / 09-18，Strix AI 渗透工具两轮）

内部三轮审计之后，先后进行两轮独立外部渗透测试（AI Agent 红队，OWASP WSTG / PTES 方法论，
CVSS v3.1 评分），所有发现均已修复并复测闭环。

### 第一轮（白盒代码扫描 + 黑盒攻击）

白盒扫描给出 4 项 Immediate、5 项 Short-term、4 项 Medium-term 建议，与本项目已知遗留高度
重合。当日修复（commit 7d54cb9 / 03a3d43）：

| 发现 | 处置 |
|------|------|
| 登录回跳开放重定向（`//evil.com` 等变体可绕过字符串前缀判断） | safeNext 控制字符剥离 + URL 解析器同源校验 |
| 会话密钥占位值可照抄上线 | 已知占位值 / <32 字符直接拒绝启动，示例文件置空 |
| 管理员默认口令可从文档获得 | 生产生成一次性随机口令，开发口令仅限本机回环 |
| 组队邀请作废被事务回滚（仍占名额、可重放接受） | 作废标记随事务提交，提交后再抛 409 |
| 公共任务打卡状态被任意用户推进 | 状态副作用收权至 owner / 管理员 |
| 注册时序 / 并发枚举 | 重复 409 同形 + 等量 bcrypt 等时 |
| 删除任务遗留孤儿照片 | 级联清理磁盘文件 |
| 列表分页非整数参数致 500 | 统一取整钳制 |
| qs 依赖告警 | overrides 钉至修复版 6.16.0 |

随后 Day 16–18 的响应式 / 深色模式 / 照片鉴权访问（uploads 移出 public 静态直出）陆续落地。

### 第二轮（纯黑盒复测：1 高 / 1 中 / 1 低，全部处置）

复测确认第一轮修复全部生效，且注入、XSS（含 CSP 约束下的 DOM 类）、CSRF（Origin/Referer
校验 + SameSite）、文件上传（魔数 / 尺寸深检 / 穿越越权 20+ 变体）、越权（IDOR/BOLA）、
会话伪造与撤销（1.7M 候选密钥爆破未果）、暴破限流、开放重定向等高危害类全部稳固。

| 发现 | 严重度 | 处置（commit 6505987） |
|------|--------|------------------------|
| 运行实例存在默认管理员口令 | High 8.2 | 定性为部署问题（开发实例暴露局域网）：`HOST` 环境变量，开发模式默认绑 127.0.0.1；生产部署要点固化（NODE_ENV + SESSION_SECRET + ADMIN_PASSWORD 三件套） |
| 单人打卡无每日一次约束（统计可刷量） | Medium 4.3 | 对齐双人口径：checkins 补 day 列 + 唯一索引 + 服务层 409，迁移先去重后建索引 |
| 照片仅鉴权无逐文件属主校验 | Low 3.1 | 判定 by design：全部上传类别经广场 / 搭档向登录用户展示，边界即「登录可见」，已文档化 |

附带收敛：错误中间件尊重 err.status（400/413 不再一律 500）、请求体 1MB 上限、双人打卡
隐私文案对齐广场实际可见性。

### 最终状态（v1.0.0）

- 两层回归网：`dbCheck.js` 服务层 59 项 + `httpCheck.js` HTTP 层 25 项（`npm run verify`）。
- 遗留 P2（有意延后，非缺陷）：Express 5 迁移、图片全量解码 / 病毒扫描、注册枚举响应文案
  （产品取舍：需向用户反馈「账号已存在」，时序与并发侧信道已封）。
- 结论：两轮外部渗透测试发现全部闭环，达到交付标准。
