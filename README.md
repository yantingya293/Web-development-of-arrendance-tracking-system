...
│   ├── js/                # 前端脚本（app.js 公共交互 / auth.js 认证表单 / tasks.js 单人任务板块）
...
当前进度：**Day 5 · 单人任务模块上线（M1 进行中）**。

- ✅ Day 2：SQLite 数据层 —— 七张表（users / tasks / checkins / teams / duo_tasks / duo_checkins / notifications）+ `src/db/db.js` 封装 + 种子数据；运行 `node dbCheck.js` 可验证（20 项检查）
